...
class BackupRotationTest {
    ...
    fun `backup file names are self-describing`() {
        assertEquals("spapp-backup-v7.apk", BackupRotation.backupFileName(7))
    }

    @Test
    fun `after a successful update the older backup is pruned, the previous version kept`() {
        val stale = BackupRotation.staleBackups(
            listOf("spapp-backup-v1.apk", "spapp-backup-v2.apk"),
            currentVersionCode = 3,
        )

        assertEquals(listOf("spapp-backup-v1.apk"), stale)
    }

    @Test
    fun `several stale generations are all pruned at once`() {
        val stale = BackupRotation.staleBackups(
            listOf("spapp-backup-v1.apk", "spapp-backup-v2.apk", "spapp-backup-v3.apk"),
            currentVersionCode = 4,
        )

        assertEquals(listOf("spapp-backup-v1.apk", "spapp-backup-v2.apk"), stale)
    }
    ...
    fun `backups at or above the running version are never pruned`() {
        val stale = BackupRotation.staleBackups(
            listOf("spapp-backup-v5.apk", "spapp-backup-v9.apk"),
