...
> places (the path is `POST /face/verifications`, the capture block is `capture`, and a pass now issues a single-use `verificationId`). What still holds is the *client-side* reasoning — which outcomes are
